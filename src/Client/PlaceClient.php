...
class PlaceClient extends AbstractClient
{
    ...
    {
        return $this->detailsAsync($placeid, $language, $params)->wait();
    }

    /**
     * Returns the details of a place (asynchronous).
     *
     * @see https://developers.google.com/places/documentation/details
     * @see https://spreadsheets.google.com/pub?key=p9pdwsai2hDMsLkXsoM05KQ&gid=1
     *
     ...
     */
    public function detailsAsync($placeid, $language = 'en', $params = [])
    {
    ...
    {
        return $this->photoAsync($reference, $params)->wait();
    }

    /**
     * Returns a photo content (asynchronous).
     *
     * @see https://developers.google.com/places/documentation/photos#place_photo_requests
     *
     * @param string $reference string identifier that uniquely identifies a photo. Photo references are returned from
     ...
     * @return mixed|null
     */
    public function photoAsync($reference, $params = [])
    {
        ...
        $promise = $this->getGuzzleClient()->getAsync($url, ['query' => $params]);
    ...
    {
        return $this->addAsync($location, $name, $types, $accuracy, $language, $params)->wait();
    }

    /**
     * Adds a place on Google's places database for your application. This function only works with JSON formats, that
     * means that no matter what you set the [[$format]] to work with, it will be superseded by 'json' type (asynchronous).
     *
     * @see https://developers.google.com/places/documentation/actions#adding_a_place
     *
     * @param array  $location The textual latitude/longitude value from which you wish to add new place information.
     ...
     * @return PromiseInterface
     */
    public function addAsync(array $location, $name, array $types, $accuracy, $language = 'en', array $params = [])
    {
        if (strlen($name) > 255) {
    ...
    {
        return $this->deleteAsync($reference)->wait();
    }
     ...
     * @return PromiseInterface
     */
    public function deleteAsync($reference)
    {
        return $this->request(
            'delete',
            'post',
